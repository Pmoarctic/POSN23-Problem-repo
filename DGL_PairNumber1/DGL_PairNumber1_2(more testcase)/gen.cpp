...
string folderName = "DGL_PairNumber1_2";
long long ITEM = 1;
...
void Process(ofstream& in, ofstream& out)
{
    ...
    int n = random(1000000,100000,in);
    map<int,int> mp;
    for(int i=0;i<n;i++)
    {
        int temp = random(100,20000,in);
        mp[temp]++;
    }
    int q = random(1000,1000,in);
    for(int i=0;i<q;i++)
    {
        int sum = random(0,40000,in);
        int check=0;
        ...
        for(it=mp.begin();it!=mp.end();it++)
        {
            ...
            if(it->first==sum)
                continue;
            if(it->first==sum-it->first&&it->second<2)
                break;
